...
%\newcommand{\mycommand}{} % Use new commands only as aliases to longer expressions
...
\title{Template for 2SPL letters}
\thanks{Project under the mentorship of}% Facultative
...
\begin{abstract}
    The abstract should contain a concise summary of the motivation, the content and the results of the article.
\end{abstract}
...
The introduction should give the context of the article and cite relevant scientific references~\cite{wheeler1949electrodynamics,beaudoin2024extending,landauQuantum} ({\em all} relevant sources are expected to be cited in the text, but not necessarily in the introduction). All sources that appear in the bibliography must be cited in the text.
...
Particularly long equations or diagrams can be placed in a \verb|widetext| environment:
\begin{widetext}
\begin{equation}
\begin{tikzcd}
    & (A \otimes B) \otimes (C \otimes D) \arrow[dr, "\alpha_{A \otimes B,C,D}"] & \\
    A \otimes (B \otimes (C \otimes D)) \arrow[ur, "\alpha_{A,B,C \otimes D}"] \arrow[d, "{\rm id}_A \otimes \alpha_{B,C,D}"] & & ((A \otimes B) \otimes C) \otimes D \\
    A \otimes ((B \otimes C) \otimes D) \arrow[rr, "\alpha_{A,B \otimes C,D}"] & & (A \otimes (B \otimes C)) \otimes D \arrow[u, "\alpha_{A,B,C} \otimes {\rm id}_D"].
\end{tikzcd}
\end{equation}
\end{widetext}
...
The conclusion should be brief and summarise the main messages of the article, as well as possible future explorations.
